...
function createImageFromVmAndWait(data, options, account, callback) {
    ...
    fn.call(self, function (err, job, res) {
        if (err) {
            ...
            waitForJob(req, wfapiUrl, job['job_uuid'], function (jErr) {
                if (jErr) {
                    callback(jErr);
                    return;
                }
        ...
        retry.on('ready', function (number, delay) {
            ...
            self.client.post(reqOpts, function (err, req, res, obj) {
                if (err) {
                    ...
                    return waitForJob(options.req, wfapiUrl, obj.job_uuid,
                        function (jErr) {
 ...
 */
function waitForJob(req, url, job_uuid, cb) {
    ...
    // WFAPI CLIENT
    //
    // NOTE: This client is "special" because node-sdc-clients can be used by
    // more than just Triton tools. So we need to use the URL that IMGAPI
    // returned for us and create a new client each time. We'll then swap this
    // out for a child if a req was passed in which case we can add tracing
    // headers.
    var client = restify.createJsonClient({url: url, agent: false});

    // replace with traced version
    if (req) {
        client = Tracer.restifyClient.child(client, req);
    }
...
IMGAPI.prototype.child = function child(req) {
    ...
    _child.client = Tracer.restifyClient.child(self.client, req,
        'sdc-clients:imgapi');
    _child.rawClient = Tracer.restifyClient.child(self.rawClient, req,
        'sdc-clients:imgapiRaw');
