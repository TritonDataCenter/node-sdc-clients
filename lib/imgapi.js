 ...
 * @param {Function} callback : `function (err, image, res)`
 */
IMGAPI.prototype.addImageFile = function addImageFile(uuid, filePath, user,
                                                      callback) {
    assert.string(uuid, 'uuid');
 ...
 */
IMGAPI.prototype.getImageFile = function getImageFile(uuid, filePath, user,
                                                      callback) {
    assert.string(uuid, 'uuid');
    assert.string(filePath, 'filePath');
    if (callback === undefined) {
 ...
 * @param {String} uuid : the UUID of the image.
 * @param {UUID} user : Optional. The UUID of the user on behalf of whom this
 *      request is being made. If given this will restrict to images
 *      accessible to that user.
 * @param {Function} callback : `function (err, res)`
 */
IMGAPI.prototype.deleteImage = function deleteImage(uuid, user, callback) {
    assert.string(uuid, 'uuid');
    if (callback === undefined) {
        callback = user;
        user = undefined;
