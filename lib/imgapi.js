...
function IMGAPI(options) {
    ...
    if (options.password) {
        ...
        assert.string(options.user, 'options.privKey, but no options.user');
 ...
 *
 * @param {String} uuid : the UUID of the image.
 * @param {String|Object} file : Readable stream or path to the image file.
 ...
 * @param {Function} callback : `function (err, image, res)`
 */
IMGAPI.prototype.addImageFile = function addImageFile(uuid, file, account,
                                                      callback) {
    var self = this;
    assert.string(uuid, 'uuid');
    assert.ok(['string', 'object'].indexOf(typeof(file)) !== -1, 'file');
    if (callback === undefined) {
        callback = account;
        account = undefined;
    }
    ...
    function getFileStreamAndStats(next) {
    ...
    getFileStreamAndStats(function (err, stream, stats) {
        if (err) {
            callback(err);
            return;
        }
