 ...
 *      - `privKey` {Buffer} Optional. RSA private key content to use for
 *        signing for signature auth.
 */
function IMGAPI(options) {
    ...
    //TODO: optionalBuffer assert-plus doesn't work here.
    assert.optionalObject(options.privKey, 'options.privKey');

    this.client = restify.createJsonClient(options);
    this.rawClient = restify.createClient(options);
    if (options.privKey) {
        assert.string(options.username,
            'options.privKey, but no options.username');
        this.username = options.username;
        this.privKey = options.privKey;
        this.algorithm = 'rsa-sha256'; // TODO support changing this
    } else if (options.password) {
        assert.string(options.username,
            'options.password, but no options.username');
...
IMGAPI.prototype.ping = function ping(error, callback) {
    ...
    /**
     * HTTP Signature scheme signing. Return the authorization header for
     * the given details.
     */
    function sign(keyId, privKey, str, headers, algorithm) {
        var signer = crypto.createSign(algorithm.toUpperCase());
        signer.update(str);
        var signature = signer.sign(privKey, 'base64');
        return format('Signature keyId="%s",algorithm="%s" %s',
            keyId, algorithm, signature);
    }
    var headers = {};
    if (this.privKey) {
        headers.date = new Date().toUTCString();
        headers.authorization = sign(this.username, this.privKey,
            headers.date, "Date", this.algorithm);
    }
    var opts = {
        path: path,
        headers: headers
    }
    this.client.get(opts, function (err, req, res, pong) {
        if (err) {
