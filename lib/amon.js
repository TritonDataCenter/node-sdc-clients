...
var CONTACT_BASE_FMT = USER_FMT + '/contacts';
var CONTACT_FMT = CONTACT_BASE_FMT + '/%s';
 ...
 * @param {Function} callback : call of the form f(err, user)
 */
AMON.prototype.getUser = function(user, callback) {
  if (!user) {
...
/**
 * Lists contacts by customer
 *
 * @param {String} customer : the customer uuid.
 * @param {Function} callback : call of the form f(err, contacts)
 */
AMON.prototype.listContacts = function(customer, callback) {
  if (!customer) {
  ...
  var self = this;
  var request = {
    path: sprintf(CONTACT_BASE_FMT, customer)
  };
...
/**
 * Gets contact by customer and contact name.
 *
 *
 * @param {String} customer : the customer uuid.
 * @param {String} contactName : the name of the contact.
 * @param {Function} callback of the form f(err, contact).
 */
AMON.prototype.getContact = function(customer, contactName, callback) {
  if (!customer) {
    throw new TypeError('customer is required');
  }
  
  if (!contactName) {
    throw new TypeError('contactName is required');
  }
  ...
  var self = this;
  var request = {
    path: sprintf(CONTACT_FMT, customer, contactName)
  };
 ...
 * @param {Object}  contact : The customer contact information. Should contain 
 * the following
 * [
 *    {
 *      "name"   : "my-email",
 ...
 *
 */
AMON.prototype.createContact = function(customer, contact, callback) {
  if (!customer) {
    throw new TypeError('customer is required');
  }
  
  if (!contact || typeof contact !== 'object') {
    throw new TypeError('contact is required (object)');
  }
  
  if (!contact.name || !contact.medium || !contact.data) {
    throw new TypeError('contact must contain name, medium and data fields');
  }
  this._validateCallback(callback);
  
  var self = this;
  var request = {
    path: sprintf(CONTACT_FMT, customer, contact.name),
    body: contact
  };
...
/**
 * Deletes a contact from AMON by contact name.
 *
 * @param {String} customer : the customer uuid.
 * @param {String} contactName : the name of the contact
 * @param {Function} callback of the form f(err).
 */
AMON.prototype.deleteContact = function(customer, contactName, callback) {
  if (!customer) {
    throw new TypeError('customer is required');
  }
 ...
 * @param {String} customer : Customer uuid.
 * @param {Object}  monitor : The monitor, should contain 
 * the following
 * [
 *    {
 *      "name"   : "my-monitor",
 ...
 *
 */
AMON.prototype.createMonitor = function(customer, monitor, callback) {
  if (!customer) {
    throw new TypeError('customer is required');
  }
 ...
 * @param {String} customer : The customer UUID.
 * @param {String} monitornName : The name of the monitor.
 ...
 * }
 */
AMON.prototype.createProbe = function(customer, monitorName, probe, callback) {
  if (!customer) {
    throw new TypeError('customer is required');
  }
