 ...
 *      - headers {Object} Optional. Additional headers to include in MAPI
 *        requests.
 *      - logLevel
 *      - errorFormatter {Function} Optional. By default errors are
 *        returned from MAPI unchanged. You can specify a
 *        `function (err) -> err` function to translate errors or use
 *        the built-in `restifyErrorFormatter`.
 */
function MAPI(options) {
  ...
  this.errorFormatter = options.errorFormatter || noopErrorFormatter;

  this.client = restify.createClient(options);

  // In-memory caches
  if (options.pkgCache !== false)
    this.pkgCache = cache.createCache(options.pkgCache);

  if (options.datasetCache !== false)
    this.datasetCache = cache.createCache(options.datasetCache);
}
...
MAPI.prototype.listNetworks = function(customer, options, callback) {
  ...
  this.client.get('/networks', function(err, networks) {
    if (err)
      return callback(self.errorFormatter(err));
...
MAPI.prototype.listDatasets = function(customer, options, callback) {
  ...
  if (this.datasetCache) {
    var cached = this.datasetCache.get(request.path);
    if (cached)
      return callback(null, cached);
  }

  return this.client.get(request, function(err, datasets) {
    if (err)
      return callback(self.errorFormatter(err));

    if (self.datasetCache)
      self.datasetCache.put(request.path, datasets || []);
...
MAPI.prototype.getDataset = function(customer, dataset, callback) {
  ...
  return this.client.get(request, function(err, obj, headers) {
    ...
    if (self.datasetCache)
      self.datasetCache.put(request.path, obj);
...
MAPI.prototype.listPackages = function(customer, options, callback) {
  ...
  if (this.pkgCache) {
    var cached = this.pkgCache.get(request.path);
    if (cached)
      return callback(null, cached);
  }

  return this.client.get(request, function(err, packages) {
    if (err)
      return callback(self.errorFormatter(err));

    if (self.pkgCache)
      self.pkgCache.put(request.path, packages || []);
...
MAPI.prototype.getPackageByName = function(customer, name, callback) {
  ...
  var cacheKey = request.path + '?find_with=name';
  if (this.pkgCache) {
    var cached = this.pkgCache.get(cacheKey);
    if (cached)
      return callback(null, cached);
  }
  ...
  return this.client.get(request, function(err, obj, headers) {
    ...
    if (self.pkgCache)
      self.pkgCache.put(cacheKey, obj);
 ...
 *
 * @param {Object} options Any given options are passed as query params to
 *    MAPI. E.g. use `owner_uuid=:uuid` to limit to machines owned by that
 *    user. Others options include: status, ram, dataset, server_uuid...
 *    See <https://head.no.de/docs/mapi/#GET-/machines> for full details.
 * @param {Function} callback of the form `function (err, count, headers)`.
 */
MAPI.prototype.countMachines = function(options, callback) {
  if (typeof(options) === 'function') {
    callback = options;
    options = {};
  }
  var self = this;
  this.listMachines(options, function (err, _, headers) {
    if (err) return callback(self.errorFormatter(err));
    var count = Number(headers['x-joyent-resource-count']);
    callback(null, count, headers);
  }, true);
};
 ...
 * @param {Object} options Any given options are passed as query params to
 *    MAPI. E.g. use `owner_uuid=:uuid` to limit to machines owned by that
 *    user. Others options include: limit, offset, status, ram, dataset,
 *    server_uuid...
 *    See <https://head.no.de/docs/mapi/#GET-/machines> for full details.
 * @param {Function} callback `function(err, machines, headers)` where
 *    `machines` is null if `headOnly=true`.
 * @param {Boolean} headOnly HEAD instead of GET (optional, default false).
 */
MAPI.prototype.listMachines = function(options, callback, headOnly) {
  if (typeof(options) === 'function') {
    callback = options;
    options = {};
  }
  ...
  if (options && options.owner_uuid)
    request.headers.User = options.owner_uuid;
...
/**
 * Retrieves a particular machine by user and alias. Aliases are unique
 * per *user*.
 *
 * Note this call will return the machine regardless of what state it is in,
 * including if it's destroyed, so check the `running_status`.
 *
 * @param {String} user The user UUID to which to limit.
 * @param {String} alias the machine alias.
 * @param {Function} callback of the form f(err, machine).
 */
MAPI.prototype.getMachineByAlias = function(user, alias, callback) {
  ...
  return this.client.get(request, function(err, obj, headers) {
    ...
    if (!obj) {
      return callback(newError({
        httpCode: HttpCodes.NotFound,
        restCode: RestCodes.ResourceNotFound,
        message: sprintf("A machine with alias '%s' does not exist.", alias)
      }));
    }
    return callback(null, obj);
  });
...
/**
 * Retrives a particular machine by name (uuid).
 * <https://head.no.de/docs/mapi/#GET-/machines/:uuid>
 *
 * Note this call will return the machine regardless of what state it is in,
 * including if it's destroyed, so check the `running_status`.
 *
 * @param {String} name The machine name (uuid).
 * @param {Object} options Optional limits on the query. Valid keys are:
 *    `owner_uuid` {UUID} Limit to machines owned by this user.
 * @param {Function} callback `function (err, machine)`.
 */
MAPI.prototype.getMachine = function(name, options, callback) {
  if (!name) throw new TypeError('name is required');
  if (typeof(options) === 'function') {
    callback = options;
    options = {};
  ...
  var self = this;
  var query = {};
  if (options && options.owner_uuid) query.owner_uuid = options.owner_uuid;
  var request = {
    path: sprintf("/machines/%s", name),
    query: query,
    expect: [200, 204],
    headers: {}
  };
  ...
  return this.client.get(request, function(err, machine, headers) {
    if (err)
      return callback(self.errorFormatter(err));
    if (!machine) {
      return callback(newError({
        httpCode: HttpCodes.NotFound,
        restCode: RestCodes.ResourceNotFound,
        message: sprintf("A machine with name '%s' does not exist.", name)
      }));
    }
 ...
 *
 * @param {String} customer the CAPI customer uuid.
 * @param {Object} options Optional. No currently supported options.
 * @param {Function} callback of the form `function (err, count, headers)`.
 * @deprecated since 7.0.0 - use `countMachines`
 */
MAPI.prototype.countZones = function(customer, options, callback) {
  ...
  var self = this;
  this.listZones(customer, options, function (err, _, headers) {
    if (err) return callback(self.errorFormatter(err));
    var count = Number(headers['x-joyent-resource-count']);
    callback(null, count, headers);
  }, true);
 ...
 *
 * Use `countZones` to figure out how many zones there are.
 *
 * @param {String} customer the CAPI customer uuid.
 ...
 * @param {Boolean} headOnly HEAD instead of GET (optional, default false).
 * @deprecated since 7.0.0 - use `listMachines`
 */
MAPI.prototype.listZones = function(customer, options, callback, headOnly) {
  ...
  if (options && options.offset) query.offset = options.offset;
  var request = {
 ...
 * @deprecated since 7.0.0 - use `getMachineByAlias`
 */
 ...
 * @param {Function} callback of the form f(err, zone).
 * @deprecated since 7.0.0 - use `getMachine`
 */
...
/**
 * Overwrites the zone meta data for a given zone uuid.
 *
 * @param {String} customer uuid.
 * @param {String} zone uuid.
 * @param {Object} meta key/value pairs of metadata to replace.
 * @param {Function} callback of the form f(err, headers).
 */
MAPI.prototype.putZoneMetadata = function(customer, zone, meta, callback) {
  if (!customer) throw new TypeError('customer is required');
  if (!zone) throw new TypeError('zone is required');
  if (!meta || typeof(meta) !== 'object')
    throw new TypeError('meta (object) required');
  if (!callback || typeof(callback) !== 'function')
    throw new TypeError('callback is required (function)');
...
MAPI.prototype.countVirtualMachines = function(customer, options, callback) {
  ...
  var self = this;
  this.listVMs(customer, options, function (err, _, headers) {
    if (err) return callback(self.errorFormatter(err));
    var count = Number(headers['x-joyent-resource-count']);
    callback(null, count, headers);
  }, true);
...
/**
 * Overwrites the VM meta data for a given machine uuid.
 *
 * @param {String} customer uuid.
 ...
 * @param {Function} callback of the form f(err, headers).
 */
MAPI.prototype.putVirtualMachineMetadata = function(customer,
                                                       vm,
                                                       meta,
                                                       callback) {
  if (!customer) throw new TypeError('customer is required');
...
MAPI.prototype.putVMMetadata = MAPI.prototype.putVirtualMachineMetadata;
...
function noopErrorFormatter(err) {
  return err;
}

/**
 * Translate MAPI errors to restify-style.
 */
function restifyErrorFormatter(err) {
  assert.ok(err);
  ...
  if (err.details && err.details.object) {
    ...
    case 'DuplicateAliasError':
      ...
      _err.message = 'name is already in use';
      break;
...
module.exports = MAPI;
module.exports.noopErrorFormatter = noopErrorFormatter;
module.exports.restifyErrorFormatter = restifyErrorFormatter;
