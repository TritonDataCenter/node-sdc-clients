...
var ZONE_USAGE_FMT = '/customers/%s/zones/%s/usage/%s';

var VM_ACTION_FMT = '/customers/%s/vms/%s/%s';
...
MAPI.prototype.getZoneUsage= function(customer, zname, month, callback) {
  ...
  return this.client.read(resource, function(error, usage) {
    if (error || !usage) {
...
MAPI.prototype.rebootVirtualMachine = function(customer, vm, callback) {
  ...
  var self = this;
  var request = {
    path: sprintf(VM_ACTION_FMT, customer, vm, 'rebooy'),
    expect: [202, 410]
  };
...
MAPI.prototype.deleteVirtualMachine = function(customer, vm, callback) {
  ...
  var self = this;
  var request = {
    path: sprintf('/customers/%s/vms/%s', customer, zone),
    expect: [202, 410]
  };
