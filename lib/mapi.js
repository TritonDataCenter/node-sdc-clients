 ...
 * @param {String} zname the container id.
...
MAPI.prototype.listVirtualMachines = function(customer, options, callback) {
  ...
  var path = sprintf(CUST_FMT + '/vms', customer);
  if (options) {
    if (options.limit !== undefined)
      path = path + '?limit=' + options.limit;
    if (options.offset !== undefined)
      offset = ((path.indexOf('?') === -1) ?
                path + '?offset=' : path + '&offset=') + options.offset;
  }
...
MAPI.prototype._cacheGet = function(cache, expiry, customer, name) {
  ...
  if (cached) {
    ...
    if (cached.found && cached.message instanceof Array && anon) {
      anon.message.forEach(function(p) {
        cached.message.push(p);
      });
    }
  } else if (anon) {
    cached = anon;
  ...
  if (cached && ((now - cached.ctime) > expiry))
    cached = null;

  log.trace('MAPI._cacheGet(%s): cache hit => %o', cacheKey, cached);
  return cached;
}
