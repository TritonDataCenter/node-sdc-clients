...
function Config(svc, ufdsargs, scope, nofollowref) {
  ...
  if ( scope )
    scopestring = "," + scope;
  self.client.search('svc=' + svc + ',o=smartdc' + scopestring, {scope:'sub'}, [pCtrl],
    function (err, res) {
      res.on("searchEntry", function(entry) {
        if (entry.object.objectclass == "config") {
          ...
          if (ref.hostname == undefined) {
            if (refSvc) {
              self[refSvc] = new Config(refSvc, ufdsargs, scope, true);
