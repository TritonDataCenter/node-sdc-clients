  ...
  self.client.search('svc=' + svc + ',o=smartdc', {scope:'sub'}, [pCtrl],
    function (err, res) {
      res.on("searchEntry", function(entry) {
        if (entry.object.objectclass == "config") {
        ...
        } else if ( entry.object.objectclass == "referral" && nofollowref == undefined ) {
          var refSvc = "";
          ...
          if (ref.hostname == undefined) {
            if (refSvc != '') { self[refSvc] = new Config(refSvc, ufdsargs, true);}
          } else {
            ...
            if (refSvc != '') {
              try {
                self[refSvc] = Config(refSvc, connectargs, true);
                self[refSvc].on("update", function() { self.emit("update", true); });
              } catch (err) {
