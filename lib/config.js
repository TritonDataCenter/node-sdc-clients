...
Config.prototype._makeldapchange = function(obj) {
  var self = this;
  var ret = [];
  ...
  for ( var i = 0; i < keys.length ; i++) {
    ...
    if (self[keys[i]])
      ...
      op = 'add';
    if ( keys[i].dn || keys[i].svc || typeof(obj[keys[i]]) != 'string')
      continue

    var modobject = {};
    modobject[keys[i]] = obj[keys[i]];

    ret.push( new ldap.Change({ operation: op,
      modification: modobject }));
  }
  return ret;
}
...
Config.prototype.insert = function( obj, dn ) {
  ...
  for ( var i = 0; i < keys.length ; i++) {
    ...
    changeobj.dn = dn;
    changeobj.objectclass = 'config';
    changeobj.svc = this._svc;
    ...
    self._client.add(dn, changeobj, function(err) {
      ...
      if (err && err.name == 'EntryAlreadyExistsError') {
        var changes = self._makeldapchange(changeobj);
