 ...
 *        role configuration.
 */
Config.prototype.putFile = function putFile(file, role, options, callback) {
    var self = this;
    var log = self.log;
 ...
 */
Config.prototype.putConfig = function putConfig(config, role, callback) {
    var self = this;
 ...
 */
Config.prototype.lookupFile = function lookupFile(role, options, callback) {
    var self = this;
 ...
 */
Config.prototype.lookupConfig = function lookupConfig(role, callback) {
    var self = this;
...
function deleteConfig(service, role, options, callback) {
    var self = this;
    var log = self.log;
    ...
    ldapSearch(self.client, dn, 'one', function (err, entries) {
        ...
        if (!entries || entries.length === 0) {
            var msg = sprintf('no config for service "%s", role "%s" to delete',
                service, role);
            log.error(msg);
