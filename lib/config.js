...
function recentChanges(role, changenumber, options, cb) {
    var self = this;
    var LOG = self.log;
    ...
    var searchdn = consDN(role, options);

    var filter;
    if (options && options.zoneid) {
        filter = sprintf('(&(changenumber>=%s)' +
            '(targetdn=zoneid=%s, role=%s, ou=config, o=smartdc))',
            options.zoneid, changenumber, role);
    } else {
        filter = sprintf('(&(changenumber>=%s)' +
