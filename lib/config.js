...
function ldapSearch(client, dn, scope, cb) {
    ...
    client.search(dn, opts, function (err, res) {
        ...
        res.on('error', function (suberr) {
            if (suberr.name === 'NoSuchObjectError') {
                return (cb(null, []));
            }
...
function Config(config) {
    ...
    if (config.tlsOptions) {
        opts.tlsOptions = config.tlsOptions;
    }

    self.client = ldap.createClient(opts);
}
...
Config.prototype.lookupFile = function lookupFile(role, options, cb) {
    ...
    var lookupRoleConfig = function (_, cb) {
        ldapSearch(client, consDN(role), 'base', function (err, results) {
            if (err)
                return (cb(err));
    ...
    var lookupConfig = function (_, cb) {
        ...
        return (ldapSearch(client, consDN(role, options), 'base',
...
Config.prototype.deleteFile = function deleteFile(service, role, options, cb) {
    var self = this;
    var LOG = self.log;
...
/*
 * A simple wrapper over deleteFile().
 */
Config.prototype.deleteConfig = function deleteConfig(role, cb) {
    var self = this;
    var LOG = self.log;

    self.deleteFile(role, role, function (err) {
        if (err) {
            LOG.error('failed to delete config for role %s: %s',
                role, err.message);
        } else {
            LOG.info('delete config for role %s', role);
        }
        return (cb(err));
    });
};
