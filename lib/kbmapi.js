...
KBMAPI.prototype.createKey = function (cn_uuid, params, options, callback) {
    assert.uuid(cn_uuid, 'cn_uuid');
    assert.object(params, 'params');
    params.cn_uuid = cn_uuid;
...
KBMAPI.prototype.getKey = function (cn_uuid, options, callback) {
    assert.uuid(cn_uuid, 'cn_uuid');
    ...
    var opts = { path: format('/key/%s', cn_uuid) };
    if (options && options.headers) {
        opts.headers = options.headers;
    }
...
KBMAPI.prototype.deleteKey = function (cn_uuid, params, options, callback) {
    assert.uuid(cn_uuid, 'cn_uuid');
    ...
    var opts = { path: format('/key/%s', cn_uuid), query: params };
    if (options && options.headers) {
        opts.headers = options.headers;
    }
