...
function translateError(err) {
    ...
    if (err instanceof restify.HttpError) {
        ...
        default:
            error = new restify.InternalError(err, err.message);
            break;
        }
    } else {
        error = new restify.InternalError(err, err.message);
...
UFDS.prototype.authenticate = function authenticate(login, passwd, cb) {
    ...
    var cacheKey = (login.login || login) + ':' + passwd;
    var self = this;
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype._getUser = function _getUser(filter, msg, cb, noCache) {
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getUser = function getUser(login, cb, noCache) {
    assert.func(cb, 'callback');
    if (typeof (login) !== 'object') {
        assert.string(login, 'login');
    } else {
    ...
    var filter = sprintf(
            '(&(objectclass=sdcperson)(|(login=%s)(uuid=%s)))',
            login, login);
    var msg = login + ' does not exist';

    this._getUser(filter, msg, cb, noCache);
};
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getUserByEmail = function getUserByEmail(email, cb, noCache) {
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getKey = function getKey(user, fp, cb, noCache) {
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listKeys = function listKeys(user, cb, noCache) {
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
    }
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listLimits = function listLimits(user, cb, noCache) {
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
    }
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getLimit = function getLimit(user, dc, cb, noCache) {
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
    }
    if (typeof (dc) !== 'string') {
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getMetadata = function getMetadata(user, key, cb, noCache) {
    if (typeof (user) !== 'string') {
        assert.object(user, 'user');
    }
    assert.string(key, 'key');
