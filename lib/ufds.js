...
UFDS.prototype.listForeigndc = function listForeigndc(user, authdev, callback) {
  ...
  self.search(dn, opts, function(err, dclist) {
    if (err)
      callback(err);
    callback(null, dclist);
  });
}


/**
 * Retrieves foreign dc by authorized dev & key.
 *
 * @param {Object} user the object you got back from getUser.
 * @param {String} authdev the authorized developer key
...
UFDS.prototype.getForeigndc = function getForeigndc(user, authdev, dcname, callback) {
}
*/
...
UFDS.prototype.addForeigndc = function addForeigndc(user, authdev, dc, callback) {
   var self = this;
   var opts = {
    scope: 'one',
  ...
  self.search(user.dn, opts, function(err, devlist) {
    ...
    if (devlist.length != 0) {
      ...
      var dcopts = {
        scope: 'one',
        filter: sprintf('(&(objectclass=foreigndc)(foreigndc=%s))', dc.name)
      };
      ...
      self.search(dcdn, dcopts, function(err2, dclist) {
        ...
        if (dclist.length != 0) {
          // The dc already exists, so we're replacing the token and/or url
          var changes = [
            { type: 'replace',
          ...
          var moddn = sprintf(FOREIGNDC_FMT, dc.name, authdev, user.uuid);
          self.modify(moddn, changes, callback);
        } else {
          var insertdn = sprintf(FOREIGNDC_FMT, dc.name, authdev, user.uuid);
          var obj = {
                      foreigndc: dc.name,
                      url: dc.url,
                      token: dc.token,
          ...
          self.add(insertdn, obj, callback);
        }
      });
