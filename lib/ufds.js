 ...
 *
 * @param {String|Object} The user UUID or login string or a user object with
 *      a `user.dn`, `user.uuid` or `user.login` (i.e. a user object as from
 *      `getUser`).
 * @param {Object} changes the plain object you want merged in. For example:
...
UFDS.prototype.updateUser = function updateUser(user, changes, callback) {
    ...
    getDn(user, function (err, dn) {
        ...
        Object.keys(changes).forEach(function (k) {
            ...
            var change;
            if (changes[k] === null) {
                change = {type: 'delete', modification: {k: []}};
            } else {
                change = {type: 'replace', modification: {k: changes[k]}};
            }
