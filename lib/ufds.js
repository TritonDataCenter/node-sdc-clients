    ...
    self.client.compare(user.dn, 'userpassword', password, function(err, ok) {
      ...
      if (!ok)
        return callback(newError({
          httpCode: HttpCodes.NotAuthorized,
...
UFDS.prototype.addUser = function addUser(user, callback) {
  ...
  this.add(sprintf(USER_FMT, user.uuid), user, function(err) {
    if (err)
      return callback(err);

    self.getUser(user.uuid, function(err, user) {
      if (err)
        return callback(err);
...
UFDS.prototype.getUser = function getUser(login, callback) {
  ...
  // This is a little goofy; the .search API returns a boolean indicating
  // if the search was served out of cache.  Here we use it to see if we don't
  // even need to try an alternate query
  if (!this.search(USERS, optsLogin, _callback)) {
    var optsUuid = {
      scope: 'base',
      attributes: ['*', 'memberof'],
  ...
  return this.listKeys(user, function(err, keys) {
    ...
    if (!key) {
      return callback(newError({
        httpCode: HttpCodes.NotFound,
        restCode: RestCodes.ResourceNotFound,
        message: fingerprint + ' does not exist'
      }));
    }
  ...
  function _delKey(user, key) {
    if (!ldap.parseDN(user.dn).parentOf(key.dn)) {
      return callback(newError({
        ...
        message: key.dn + ' is not a child of ' + user.dn
      }));
    }
  ...
  function _limits(user) {
    self.listLimits(user, function(err, limits) {
      ...
      return callback(newError({
        httpCode: HttpCodes.NotFound,
        restcode: RestCodes.ResourceNotFound,
        message: 'No limit found for ' + user.login + '/' + datacenter + '.'
      }));
    });
  }
...
UFDS.prototype.add = function add(dn, entry, callback) {
  ...
  this.client.once('error', function(err) {
    log.error('UFDS client error event: %s', err.stack);
    return callback(self._translateError(err));
  });
...
UFDS.prototype.search = function search(base, options, callback) {
  ...
  this.client.search(base, options, HIDDEN, function(err, res) {
    if (err)
      return callback(self._translateError(err));
...
UFDS.prototype._translateError = function _translateError(error) {
  ...
  if (error instanceof ldap.LDAPError) {
    ...
    case 'NotAllowedOnRdnError':
      httpCode = HttpCodes.Conflict;
      restCode = RestCodes.InvalidArgument;
      message = 'The resource in question has "child" elements'.
        break;
    }
  }

  var e = newError({
    httpCode: httpCode || HttpCodes.InternalError,
    restCode: restCode || RestCodes.InternalError,
    message: message || 'An internal error occurred'
  });

  log.debug('Translated %s to %s', error.toString(), e.toString());
  return e;
};
