 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.addPackage = function addPackage(pkg, callback) {
    ...
    return self.add(sprintf(PKG_FMT, pkg.uuid), pkg, function (err) {
        if (err) {
            return callback(err);
        }

        return self.getPackage(pkg.uuid, function (err, pkg) {
            if (err) {
                return callback(err);
            }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getPackage = function getPackage(urn, callback) {
    ...
    return self.search(PACKAGES, opts, function (err, entries) {
        var pkg, msg;
        if (err) {
            return callback(err);
        }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.updatePackage = function updatePackage(pkg, changes, callback) {
    assertObject('pkg', pkg);
    assertObject('changes', changes);
    assertFunction('callback', callback);
    ...
    function _callback(pkg) {
        ...
        return self.modify(pkg.dn, _changes, callback);
    }
    ...
    return self.getPackage(pkg, function (err, pkg) {
        if (err) {
            return callback(err);
        }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.deletePackage = function deletePackage(pkg, callback) {
    assertObject('pkg', pkg);
    assertFunction('callback', callback);

    var self = this;
    function _callback(pkg) {
        return self.del(pkg.dn, callback);
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listPackages = function listPackages(callback) {
    assertFunction('callback', callback);
    ...
    return self.search(PACKAGES, opts, function (err, pkgs) {
        if (err) {
            return callback(err);
        }
    ...
    if (error instanceof ldap.LDAPError) {
        ...
        case 'NotAllowedOnRdnError':
            return new restify.InvalidArgumentError(
                'The resource in question has "child" elements');
