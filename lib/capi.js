...
function CAPI(options) {
  ...
  this.client = restify.createClient({
    ...
    url: options.url,
    headers: {
      Authorization: utils.basicAuth(options.username, options.password)
    }
  });
...
CAPI.prototype.getAccount = function(username, callback) {
  ...
  return this.client.get(request, function(err, obj, headers) {
    ...
    self._accountCachePut(username, obj);
  ...
  return this._getSalt(username, password, function(err, salt) {
    ...
    return self.client.post(loginRequest, function(err, obj, headers) {
      if (err || !obj || !obj.customer_id) {
        var e = newError({
          httpCode: 403,
          restCode: Constants.InvalidCredentials,
...
CAPI.prototype._accountCacheGet = function(username) {
  ...
  if (cached) {
    if (((new Date()).getTime() - cached.ctime) <= this.accountCacheExpiry) {
      log.debug('CAPI (acct cache): cache hit on %s: %o', username, cached);
      return cached;
    }
  }

  log.debug('CAPI (acct cache): cache miss on %s', username);
  return null;
};
...
CAPI.prototype._authCacheGet = function(username, password) {
  ...
  if (cached) {
    if (((new Date()).getTime() - cached.ctime) <= this.authCacheExpiry) {
      log.debug('CAPI (auth cache): cache hit on %s: %o', username, cached);
      return cached;
    }
  }

  log.debug('CAPI (auth cache): cache miss on %s', username);
  return null;
};
...
CAPI.prototype._translateError = function(err) {
  ...
  function _getMessage() {
    ...
    if (err.details && err.details.errors) {
      if (err.details.errors[0]) {
        msg = err.details.errors[0];
      } else {
        msg = err.details.errors;
      }
    }
    return msg;
  }
