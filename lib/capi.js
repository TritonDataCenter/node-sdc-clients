  ...
  return this.client.get(request, function(err, obj, headers) {
    ...
    if (obj) {
      for (var i = 0; i < obj.length; i++) {
        if (obj[i].login && obj[i].login === username) {
          self._accountCachePut('username:' + username, obj[i]);
          return callback(null, obj[i]);
        }
      }
    }
...
CAPI.prototype.updateAccount = function(account, callback) {
  ...
  return this.client.put(request, function(err, data) {
    ...
    // clobber cache.
    self._accountCacheDel('uuid:' + data.uuid);
    return callback(err, data);
  });
};
...
CAPI.prototype._accountCacheDel = function(key) {
  assert.ok(key);

  log.debug('CAPI._accountCacheDel(%s): deleting', key);
  this.accountCache.del(key);
  console.log("DEBUG-698 - deleted key successful?", this.accountCache.get(key));
  return;
};
