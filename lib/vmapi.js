 ...
 */
function waitForJob(req, wfapiUrl, jobUuid, cb) {
    ...
    // more than just Triton tools. So we need to use the URL that VMAPI
...
function createVmAndWait(params, options, callback) {
    ...
    self.createVm(params, options, function (err, jobInfo, res) {
        ...
        waitForJob(req, wfapiUrl, jobInfo['job_uuid'], function (jErr, job) {
            if (jErr) {
                callback(jErr);
                return;
            }
...
VMAPI.prototype.child = function child(req) {
