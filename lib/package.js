...
Package.prototype.add = function add(pkg, callback) {
    ...
    if (pkg.owner_uuid && typeof (pkg.owner_uuid) !== 'string') {
        pkg.owner_uuid = JSON.stringify(pkg.owner_uuid);
    }
...
Package.prototype.get = function get(uuid, callback) {
    ...
    return self.ufds.search(PACKAGES, opts, function (err, entries) {
        ...
        if (pkg.owner_uuid) {
            try {
                pkg.owner_uuid = JSON.parse(pkg.owner_uuid);
            } catch (e) {
                pkg.owner_uuid = null;
            }
        }
...
Package.prototype.update = function update(pkg, changes, callback) {
    ...
    if (changes.owner_uuid && changes.owner_uuid.length) {
        changes.owner_uuid = JSON.stringify(changes.owner_uuid);
    }
