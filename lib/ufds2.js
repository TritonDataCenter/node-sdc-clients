 ...
 * role and policies. It requires CAPI-305 to be finished and deployed into
...
UFDS.prototype.addPolicy = function addPolicy(account, policy, cb) {
    ...
    if (typeof (policy.rule) !== 'string') {
        assert.object(policy.rule, 'policy.rule');
        policy.rule = JSON.stringify(policy.rule);
    }
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listRoles = function listRoles(account, cb, noCache) {
    assert.string(account, 'account');
    assert.func(cb, 'callback');
