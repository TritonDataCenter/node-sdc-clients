...
// Account roles
var ROLE_FMT = 'role=%s, ' + USER_FMT;
// Account groups
var SUBGRP_FMT = 'group=%s, ' + USER_FMT;
...
UFDS.prototype.addRole = function addRole(account, role, cb) {
    ...
    var dn = sprintf(ROLE_FMT, role.role, account);
    ...
    this.add(dn, entry, function (err) {
        if (err) {
            ...
            self.getRole(account, role.role, cb);
        }
    });
};
...
UFDS.prototype.getRole = function getRole(account, role, cb) {
    ...
    var opts = {
        scope: 'one',
        filter: sprintf('(&(role=%s)(objectclass=sdcaccountrole))', role)
    };
 ...
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} role the name of the role to modify.
...
UFDS.prototype.addGroup = function addGroup(account, group, cb) {
    ...
    var dn = sprintf(SUBGRP_FMT, group.cn, account);
    ...
    this.add(dn, entry, function (err) {
        if (err) {
            ...
            self.getGroup(account, group.cn, cb);
        }
    });
};
...
UFDS.prototype.getGroup = function getGroup(account, group, cb) {
    ...
    var opts = {
        scope: 'one',
        filter: sprintf('(&(cn=%s)(objectclass=sdcaccountgroup))', group)
    };
 ...
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} group the name of the group to modify.
 * @param {Object} changes the modifications to be inserted
 * @param {Function} callback of the form fn(err, group).
 * @throws {TypeError} on bad input.
 */
