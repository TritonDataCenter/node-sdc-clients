 ...
 *
 * This is the new UFDS client, with support for multiple account users,
 * group and roles. It requires CAPI-305 to be finished and deployed into
 * production before we can use.
 *
 * THIS IS A WIP, PLEASE, DO NOT USE UNTIL CAPI-324 IS FINISHED.
 *
 */
...
// Account groups
var SUBGRP_FMT = 'group-uuid=%s, ' + USER_FMT;
...
function extendUser(self, user) {
    ...
    user.addToGroup = function addToGroup(group, account, cb) {
        if (typeof (account) === 'function') {
            cb = account;
            account = null;
        }

        var rdn = (account) ? sprintf(SUBGRP_FMT, group, account) :
            sprintf(GROUP_FMT, group);
    ...
    user.removeFromGroup = function removeFromGroup(group, account, cb) {
        if (typeof (account) === 'function') {
            cb = account;
            account = null;
        }
    ...
    user.addToRole = function addToRole(role, account, cb) {
        if (typeof (account) === 'function') {
            cb = account;
            account = user.account ? user.account : user.uuid;
        }
        var rdn = sprintf(ROLE_FMT, role, account);
        if (user.roles.indexOf(rdn) !== -1) {
            cb(null);
            return;
        }
    ...
    user.removeFromRole = function removeFromRole(role, account, cb) {
        if (typeof (account) === 'function') {
            cb = account;
            account = user.account ? user.account : user.uuid;
        }
...
UFDS.prototype.authenticate = function authenticate(login, pwd, account, cb) {
    ...
    function _compare(user) {
        client.compare(user.dn, 'userpassword', pwd, function (err, ok) {
            if (err) {
                cb(translateError(err));
            } else if (!ok) {
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype._getUser = function _getUser(filter, msg, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    this.search(base, opts, function (err, entries) {
        ...
        opts = {
            ...
            filter: sprintf(
                    '(|(&(objectclass=groupofuniquenames)(uniquemember=%s))' +
                        '(&(objectclass=sdcaccountgroup)(uniquemember=%s)))',
                    entries[0].dn.toString(), entries[0].dn.toString())
        };
        self.search(GROUPS, opts, function (groupErr, groups) {
            ...
            self.search(USERS, opts, function (rolErr, roles) {
                if (rolErr) {
                    cb(rolErr);
                    return;
                }

                entries[0].roles = roles.map(function (r) {
                    return (r.dn);
                });
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getUser = function getUser(login, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    this._getUser(filter, msg, account, cb);
};
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getUserByEmail = function getUserByEmail(email, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getKey = function getKey(user, fp, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    // TODO(trentm): This should *search* for the key, not list all and whittle.
    this.listKeys(user, account, function (err, keys) {
        if (err) {
            cb(err);
            return;
        }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listKeys = function listKeys(user, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listLimits = function listLimits(user, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getLimit = function getLimit(user, dc, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
 ...
 *
 * Note vms usage is the same thing for the main account user and all
 * the account sub-users.
 *
 * @param {Object} user the user you got from getUser.
 * @param {String} (Optional) account uuid for a customer sub-user
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listVmsUsage = function listVmsUsage(user, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
    ...
    function _vms(err, user) {
        if (err) {
            ...
            var dn = (user.account) ?
                sprintf(USER_FMT, user.account) : user.dn;
            self.search(dn, opts, cb);
        }
    }
    ...
    if (typeof (user) === 'object') {
        ...
        this.getUser(user, account, _vms);
    }
};
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getMetadata = function getMetadata(user, key, account, cb) {
    if (typeof (account) === 'function') {
        cb = account;
        account = null;
    }
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listRoles = function listRoles(account, cb) {
    assert.string(account, 'account');
    assert.func(cb, 'callback');
...
UFDS.prototype.addRole = function addRole(account, role, cb) {
    ...
    assert.string(role.role, 'role.role');

    if (typeof (role.policydocument) !== 'string') {
        assert.object(role.policydocument, 'role.policydocument');
        role.policydocument = JSON.stringify(role.policydocument);
    }
    assert.func(cb, 'callback');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getRole = function getRole(account, role, cb) {
    assert.string(account, 'account');
    ...
    var opts = {
        scope: 'one',
        filter: sprintf('(&(objectclass=sdcaccountrole)(|(uuid=%s)(role=%s)))',
                        role, role)
    };
    ...
    this.search(dn, opts, function (err, roles) {
        if (err) {
            ...
            var r = roles[0];
            if (typeof (r.policydocument) === 'string') {
                try {
                    r.policydocument = JSON.parse(r.policydocument);
                } catch (e) {
                    // Do nothing ...
                }
            }
 ...
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} role the UUID of the role to modify.
        ...
        if (err && err.statusCode === 404) {
            ...
            Object.keys(changes).forEach(function (k) {
                ...
                if (changes[k] === null) {
                    ...
                    change.type = 'replace';
                    if (k === 'policydocument') {
                        change.modification[k] = JSON.stringify(changes[k]);
                    } else {
                        change.modification[k] = changes[k];
                    }
                }
 ...
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} role the UUID of the role to retrieve.
 * @param {Function} callback of the form fn(err).
 * @throws {TypeError} on bad input.
 */
...
/**
 * Lists groups for a given account.
 *
 * @param {String} account string the uuid of main account user.
 * @param {Function} callback of the form fn(err, groups).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listGroups = function listGroups(account, cb) {
    assert.string(account, 'account');
    assert.func(cb, 'callback');
    ...
    var dn = sprintf(USER_FMT, account);
    this.search(dn, opts, cb);
};


/**
 * Creates a group for a given account
 *
 * @param {String} account string the uuid of main account user.
 * @param {Object} group the group to add.
 * @param {Function} callback of the form fn(err, group).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.addGroup = function addGroup(account, group, cb) {
    assert.string(account, 'account');
    assert.object(group, 'group');
    assert.string(group.cn, 'group.cn');
    assert.func(cb, 'callback');
    ...
    var dn = sprintf(SUBGRP_FMT, group.uuid, account);
    var entry = clone(group);
    entry.objectclass = 'sdcaccountgroup';

    this.add(dn, entry, function (err) {
        if (err) {
            cb(translateError(err));
        } else {
            self.getGroup(account, group.uuid, cb);
        }
    });
};
...
/**
 * Gets a group for the given account
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} group the UUID of the group to retrieve.
 * @param {Function} callback of the form fn(err, group).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getGroup = function getGroup(account, group, cb) {
    assert.string(account, 'account');
    ...
    var opts = {
        scope: 'one',
        filter: sprintf('(&(objectclass=sdcaccountgroup)(|(uuid=%s)(cn=%s)))',
                group, group)
    };

    this.search(dn, opts, function (err, groups) {
        if (err) {
            cb(err);
        } else {
            if (!groups.length) {
                cb(new ResourceNotFoundError(group + ' does not exist'));
                return;
            }
            cb(null, groups[0]);
        }
...
/**
 * modifies a given account group.
 *
 * takes a CAPI metadata key and an object of arbitrary fields (not nested)
 *
 *
 ...
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} group the UUID of the group to modify.
 * @param {Object} changes the modifications to be inserted
 * @param {Function} callback of the form fn(err, group).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.modifyGroup =
function modifyGroup(account, group, changes, cb) {
    assert.string(group, 'group');
    assert.string(account, 'account');
    assert.object(changes, 'changes');
    assert.func(cb, 'callback');
    ...
    var dn = sprintf(SUBGRP_FMT, group, account);
    var entry = clone(changes);
    entry.objectclass = 'sdcaccountgroup';

    self.getGroup(account, group, function (err) {
        if (err && err.statusCode === 404) {
            self.add(dn, entry, function (err2) {
                if (err2) {
                    cb(translateError(err2));
                } else {
                    self.getGroup(group, account, cb);
                }
            });
        } else {
            var ldapChanges = [];
            ...
            if (!ldapChanges.length) {
                ...
                self.modify(dn, ldapChanges, function (err3) {
                    if (err3) {
                        ...
                        self.getGroup(account, group, cb);
                    }
                });
            }
        }
...
/**
 * Deletes a group for the given account.
 *
 * @param {String} account string the uuid of main account user.
 * @param {String} group the UUID of the group to retrieve.
 * @param {Function} callback of the form fn(err).
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.deleteGroup = function deleteGroup(account, group, cb) {
    assert.string(account, 'account');
    assert.string(group, 'group');
    assert.func(cb, 'callback');
    ...
    this.del(sprintf(SUBGRP_FMT, group, account), cb);
};
