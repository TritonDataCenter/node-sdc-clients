 ...
 * group and policies. It requires CAPI-305 to be finished and deployed into
 * production before we can use.
 *
 * THIS IS A WIP, PLEASE, DO NOT USE UNTIL CAPI-324 IS FINISHED AND DEPLOYED
 * TO JPC HEADNODES.
 *
 */
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.listPolicies = function listPolicies(account, cb) {
    assert.string(account, 'account');
    assert.func(cb, 'callback');
...
UFDS.prototype.addPolicy = function addPolicy(account, policy, cb) {
    ...
    if (typeof (policy.policydocument) !== 'string') {
        assert.object(policy.policydocument, 'policy.policydocument');
 ...
 * @throws {TypeError} on bad input.
 */
UFDS.prototype.getPolicy = function getPolicy(account, policy, cb) {
    assert.string(account, 'account');
        ...
        if (err && err.statusCode === 404) {
            self.add(dn, entry, function (err2) {
                if (err2) {
                    ...
                    self.getPolicy(policy, account, cb);
                }
            });
        } else {
            var ldapChanges = [];
